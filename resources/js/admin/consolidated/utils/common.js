...
require('./../../../../../../public/js/bootstrap-datepicker.min.js');
...
(function($, w)
{
    ...
    $(function () {
		...
		carousel.bind('slid.bs.carousel', function (e) {
			...
			CHART.populateActiveChart($(this).find('.item.active .consolidated-charts').data('index'));
		});
		...
		.on('click', '#clear', function() {
            ...
            $("#predefine_dates").val('').trigger('change');
	        $('#clear_affiliates').trigger('click');
			$('#legends').multiselect('deselectAll', false).multiselect('refresh')
	    })
        // Redraw
        .on('click', '#redraw', function () {

			COMMON.fullView($('#full_view').is(':checked'));

							 // Set to show y axis label
			CHART.setYAxisLabelEnable(($('#show_yaxis').is(':checked')) ? true : false)
							 ...
							 // redraw
							 .populateActiveChart(COMMON.carouselIndex());
		})
		...
		.on('shown.bs.tab', 'a[data-toggle="tab"]', function (e) {
             ...
             if(target == '#all_affiliate') COMMON.disableAnBlur();
             else if(target == '#all_inbox') COMMON.showAllInboxForm();
             else COMMON.enableAndUnblur();
