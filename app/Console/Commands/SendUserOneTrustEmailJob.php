...
class SendUserOneTrustEmailJob extends Command
{
     ...
     * @var string
     */
    protected $signature = 'send:user-one-trust-request-email-not-job';
    ...
    public function handle(): void
    {
        ...
        //Log::info($requests);
        $this->info('Start sending...');
        Log::info('Start sending...');
        foreach ($requests as $request) {
            $this->info($request->id.' - '.$request->request_type.' - '.$request->email);
